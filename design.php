...
$need_id=$_GET["id"];
//２．データ登録SQL作成
$stmt = $pdo->prepare("SELECT * FROM design_table WHERE need_id=?");

$status = $stmt->execute(array($need_id));
...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    $view .= "<tr class='bg-danger'><td>投稿者：</td><td>".$result["lid"]."</td></tr>
            <tr><td>デザイン：</td><td><img src=".$result["image"]."></td></tr>
